...
func NewApp(
	...
	if config.Role == "master" {
		...
		getMasterInfo := grpc.NewGetMasterInfoServer(config.ClusterID, baseApp.L(), int(config.MaxOuterMessageSize), backgroundStorage, time.Duration(config.ExtensionsCacheDurabilitySec*float64(time.Second)))
